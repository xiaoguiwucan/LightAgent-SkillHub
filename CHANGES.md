...
- 修复 Skill Runner 128 MB 内存限制下并发创建多个请求线程失败的问题，管理员统计改为固定顺序读取。
- 群聊查询不再因线程栈虚拟内存不足而只返回“无法读取 Sub2API 成员用量”。
- 新增 `SUB2API_REPORT_TIMEZONE`，部署主机日期异常时可只修正该技能的统计日历，不影响其他容器。

### Sub2API 用量查询 1.0.0

- 新增 `sub2api-usage`，通过结构化 `skill_run` 入口查询 Sub2API 总体与全部当前账号的今日、本周、本月 Token、今日请求和占比。
- 账号统一使用 Sub2API 用户管理中的当前用户名，并加入今日用量最高、请求最多、前三名集中度和小时高峰分析。
- 报告只展示 Sub2API 用量，不展示 GrokBot、CloudDrive2 或机器状态；管理员地址和 API Key 仅通过容器环境变量注入。
